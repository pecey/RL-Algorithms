...
class SARSA:
    ...
    def learn(self, current_state, current_action, reward, next_state, next_action, done):
        current_value = self.Q_table[current_state][current_action]
        target = self.gamma * self.Q_table[next_state][next_action] if not done else 0
...
if __name__ == "__main__":
    ...
    sarsa = SARSA(n_states, n_actions, gamma=1)
